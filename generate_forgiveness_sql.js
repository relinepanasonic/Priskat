...
const inputCSV = fs.readFileSync('Gallery/Devotional/forgiveness_devotions.csv', 'utf8');
...
for (const row of records) {
  ...
  SELECT id INTO v_cat_id FROM public.devotion_categories
    WHERE name = '${sub_category.replace(/'/g, "''")}' AND parent_id = v_forgiveness_id
    ORDER BY created_at ASC
    LIMIT 1;
  IF v_cat_id IS NULL THEN
    INSERT INTO public.devotion_categories (name, name_id, parent_id)
      VALUES ('${sub_category.replace(/'/g, "''")}', '${sub_category_id.replace(/'/g, "''")}', v_forgiveness_id)
...
fs.writeFileSync('supabase/045_seed_devotions_forgiveness.sql', sql, 'utf8');
console.log('Done!');
