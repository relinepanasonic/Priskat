  ...
  if (language === "en") {
    ...
    while (retries > 0) {
      try {
        ...
        retries--;
        if (retries === 0) return "Failed to fetch English Bible text.";
    ...
    while (retries > 0) {
      try {
        ...
        retries--;
        if (retries === 0) return "Gagal mengambil teks Alkitab bahasa Indonesia. Server API sedang sibuk, silakan coba lagi.";
        await new Promise(r => setTimeout(r, 1000));
      }
    }
