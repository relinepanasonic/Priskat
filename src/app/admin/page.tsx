...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: "News Posts", value: postCount ?? 0, icon: Newspaper, href: "/admin/news", color: "text-brand-gold bg-brand-bg" },
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {stats.map(({ label, value, icon: Icon, href, color }) => (
          ...
          >
            ...
            <ArrowRight className="ml-auto h-4 w-4 text-brand-muted group-hover:text-brand-muted transition-colors" />
          </Link>
        ))}
      </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-brand-border bg-brand-surface p-5 shadow-sm">
          ...
          <Link href="/admin/members" className="mt-3 inline-flex items-center gap-1 text-sm text-brand-gold hover:underline">
