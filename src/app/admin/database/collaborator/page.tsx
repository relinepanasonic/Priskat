  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-white">Collaborator Data</h2>
          ...
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="text-xs uppercase bg-[#111] text-gray-400 border-b border-[#333]">
              <tr>
                ...
                <th className="px-4 py-3">Keuskupan</th>
                <th className="px-4 py-3">Payung Organisasi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#333]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="text-center py-10 text-brand-muted">No collaborators found.</td>
                </tr>
              ) : (
                data.map((row) => (
                  <tr key={row.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3 font-semibold text-white">{row.nama_komunitas}</td>
                    <td className="px-4 py-3">{row.keuskupan}</td>
                    <td className="px-4 py-3 text-brand-gold">{row.payung_organisasi}</td>
                  </tr>
                ))
              )}
            </tbody>
