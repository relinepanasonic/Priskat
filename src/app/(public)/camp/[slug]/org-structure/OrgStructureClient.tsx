...
import { Plus, Edit2, Trash2, X, Check, Search, User } from "lucide-react";
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-muted">Levels 1 to 10 within {communityName}</p>
        </div>
        {isAdmin && !showAddForm && !isEditing && (
          <button 
            onClick={() => setShowAddForm(true)}
            className="flex items-center gap-2 px-4 py-2 bg-brand-gold text-brand-dark text-sm font-bold rounded-xl hover:bg-brand-gold/80 transition-all"
          >
      ...
      {(showAddForm || isEditing) && (
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-5 mb-8">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-white">{isEditing ? "Edit Member" : "Add Member to Structure"}</h3>
            <button onClick={resetForm} className="text-brand-muted hover:text-white">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          return (
            <div key={level} className="relative">
              <div className="flex items-center gap-4 mb-6">
                ...
                <h3 className="text-brand-gold font-serif font-bold text-lg">Level {level}</h3>
                <div className="h-px bg-brand-border flex-1" />
              </div>
              ...
              {members.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {members.map(member => (
                    <div key={member.id} className="bg-brand-bg border border-brand-border rounded-2xl p-5 flex flex-col items-center text-center relative group shadow-sm hover:border-brand-gold/30 transition-colors">
                      {isAdmin && (
                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                          <button onClick={() => startEdit(member)} className="text-brand-muted hover:text-brand-gold p-1 bg-brand-dark rounded-md">
                            <Edit2 className="h-3 w-3" />
                          </button>
                          <button onClick={() => handleDelete(member.id)} className="text-brand-muted hover:text-red-400 p-1 bg-brand-dark rounded-md">
                            <Trash2 className="h-3 w-3" />
                          </button>
                        </div>
                      )}
                      
                      <div className="w-20 h-20 rounded-full overflow-hidden bg-brand-dark border-2 border-brand-gold/20 mb-3 flex items-center justify-center">
                        {member.profiles?.avatar_url ? (
                          <Image src={member.profiles.avatar_url} alt={member.profiles.full_name} width={80} height={80} className="object-cover w-full h-full" />
                        ) : (
                ...
                <div className="text-center py-8 bg-brand-bg/50 border border-dashed border-brand-border rounded-2xl">
