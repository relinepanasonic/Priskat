...
// ==========================================
// ⚠️ WARNING: DO NOT COMMIT THIS FILE ⚠️
...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZndnVheWJkbnlja2tsbnh5dnRjIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NzU4NjA4MCwiZXhwIjoyMTAzMTYyMDgwfQ.h5nqNseALAqMfUnRgvGGDv2qlBweyGcIqsO-AHq2DRM';
// ==========================================
...
function fetchChapter(version, bookNo, chapter) {
  return new Promise((resolve, reject) => {
    https.get(`https://bolls.life/get-chapter/${version}/${bookNo}/${chapter}/`, (resp) => {
      let data = '';
...
async function scrapeAndInsert(version) {
  console.log(`\n\n=== STARTING BATCH IMPORT FOR ${version} ===\n`);
  
  // 1. Delete existing records for this version
  console.log(`Clearing any existing ${version} records to prevent duplicates...`);
  await supabase.from('bible_verses').delete().eq('translation', version);
  
  // 2. Iterate through 66 books
  for (const book of ALL_BIBLE_BOOKS) {
    if (book.no > 66) continue; // Only 66 books in Protestant Bibles (NIV/NKJV)
    
    console.log(`Downloading ${version} ${book.name_en}...`);
    
    for (let c = 1; c <= book.chapter; c++) {
      try {
        const versesData = await fetchChapter(version, book.no, c);
        ...
        for (const v of versesData) {
          ...
          if (version === 'NIV') {
            ...
            if (brIndex > 0 && brIndex < 60) {
              const possibleTitle = text.substring(0, brIndex);
              if (!/[.,;:—"\'”’]$/.test(possibleTitle)) {
                title = stripHtml(possibleTitle).trim();
                text = text.substring(brIndex + 5);
              }
            }
          }
          
          // Strip remaining HTML (e.g. <i>, <b>, <br/>)
        ...
        if (error) {
          console.error(`❌ Error inserting ${version} ${book.name_en} Chapter ${c}:`, error.message);
        }
        ...
        console.error(`❌ Network error fetching ${version} ${book.name_en} Chapter ${c}:`, e.message);
      }
    }
  }
  
  console.log(`✅ Finished importing ${version}!`);
}

async function run() {
  await scrapeAndInsert('NKJV');
  await scrapeAndInsert('NIV');
  console.log('\n🎉 ALL DONE! NKJV and NIV are fully injected into Supabase!');
}
