...
// Use the `service_role` secret key to bypass RLS!
...
function getJsonName(book) {
  if (book.name_en === "Psalms") return "Psalm";
  if (book.name_en === "Wisdom of Solomon") return "Wisdom";
  return book.name_en;
}

async function run() {
  if (SUPABASE_URL.includes('YOUR_PROJECT_URL')) {
    console.error("❌ Please update SUPABASE_URL and SUPABASE_KEY in this script before running.");
    process.exit(1);
  }
  ...
  console.log("Clearing any existing NRSV-CE records to prevent duplicates...");
  await supabase.from('bible_verses').delete().eq('translation', 'NRSV-CE');

  console.log("Starting bulk import to Supabase...");
  
  for (const book of ALL_BIBLE_BOOKS) {
    const jsonName = getJsonName(book);
    const bookData = json.bible.find(b => b.book === jsonName);
    
    if (!bookData) {
      console.log(`⚠️ Skipping ${book.name_en} (not found in NRSV-CE JSON. Note: Additions to Esther/Daniel are usually merged).`);
      continue;
    }

    console.log(`Uploading ${book.name_en}...`);
    ...
    for (const chap of bookData.chapters) {
      ...
      if (error) {
        console.error(`❌ Error inserting ${book.name_en} Chapter ${c}:`, error.message);
      }
    }
  }
  
  console.log('✅ Done! All NRSV-CE books have been imported into the database.');
}
